...
async def gban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if int(user_id) in SCOUTS:
        await message.reply_text("That's a Scout! They cannot be banned!")
        return

    if int(user_id) in GARRISONS:
        await message.reply_text("That's a Garrison! They cannot be banned!")
        return
    ...
    if EVENT_LOGS:
        ...
        await send_to_list(bot, TITANSHIFTERS + ROYALS, log_message, html=True)
        ...
        try:
            ...
            if excp.message not in GBAN_ERRORS:
                ...
                if EVENT_LOGS:
                    ...
                    await send_to_list(
                        bot, TITANSHIFTERS + ROYALS, f"Could not gban due to: {excp.message}",
                    )
...
async def ungban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if EVENT_LOGS:
        ...
        await send_to_list(bot, TITANSHIFTERS + ROYALS, "un-gban complete!")
...
async def enforce_gban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        restrict_permission = member.can_restrict_members
    except Forbidden:
        return
